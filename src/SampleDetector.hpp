...
class SampleDetector
{
 ...
 public:
   ...
   //  STATUS ProcessImage(const JiImageInfo &inFrame, std::vector<BoxInfo> &result, float thresh = 0.15);
    STATUS ProcessImage(const cv::Mat &inFrame, std::vector<BoxInfo> &result, float thresh = 0.15);
 private:
    ...
    */
    ...
    void decode_outputs(float* buffer, std::vector<BoxInfo>& objects, float scale, const int img_w, const int img_h);
